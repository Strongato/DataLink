...
void print_usage(char* argv[])
{
    ...
    printf("    Usage: %s -l\n", argv[0]);
    printf("-e  - Specify employee (required for -a -u -r options)\n");
    printf("    Usage: %s -e <name,address,hours>\n", argv[0]);
    printf("-a  - Add an employee to the database file (-e option required)\n");
    printf("    Usage: %s -a\n", argv[0]);
...
int validate_args_and_print_usage(char* argv[], const unsigned short port, const char* const hostarg, const char* const employee_string, const char* const update_string, const bool remove, const bool add)
{
    ...
    if (employee_string)
    {
        if (!remove && !update_string && !add) { printf("Employee name -e is set but no additional parameters like -u, -r, -a were provided\n"); help = true; }
    }
    else
    {
        ...
        if (add) { printf("Employee name -e is a required argument when using -a\n"); help = true; }
    }
